  ...
  >
     <div class="row">
      <FormulateInput
        ...
        validation="required"
        :label="$t('currentPolicyAdmin.name')"
      />
    </div>
...
export default {
  ...
  created() {
    ...
    this.GetCurrentPolicies(d);
  },
  computed: {
  ...mapGetters(
    { Document: "StateCurrentPolicies",
  ...
  methods: {
    ...mapActions(["GetCurrentPolicies", "EditCurrentPolicy", "CreateCurrentPolicy"]),
    async sendData () {
      if (this.document != ''){
      ...
      if (this.id == 0) {
        ...
        await this.CreateCurrentPolicy(this.Document);
      } else {
        await this.EditCurrentPolicy(this.Document);
      }     
    },
  }
}
