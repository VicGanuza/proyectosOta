  ...
  <div class="text-center">
    ...
    <div v-if="!HistoricalLoading">    
      <v-img
        :aspect-ratio="16/9"
        height="450"
        src="@/assets/images/terminals/Oiltanking_South-America_Brazil_Vitoria_Tanks-Landscape_2012-06-28_10.jpg"
      ></v-img>
      <v-container class="text-justify">
        <h1 class="my-5">
          {{$t('bulkLiquidHandling.title')}}
        </h1>
        <h2 class="blue--text text--accent-4">{{$t('bulkLiquidHandling.subtitle1')}}</h2>
        <p>
        ...
        <ul>
          <li v-for="d in Documents" :key="d.id">
            {{ $t(d.name) }}
            <a v-if="d.pdf" :href="`${appPath}fileDownload.php?name=${d.pdf}`"  target="_blank">
              <v-icon color="blue" large>mdi-file-pdf-box</v-icon>
            </a>
            <a v-if="d.excel" :href="`${appPath}fileDownload.php?name=${d.excel}`"  target="_blank">
        ...
        <v-row>
          <v-select
            ...
            <v-select
              ...
              item-value="id"
              single-line
              @change="getHistoryDocuments()"
            ></v-select>
        </v-row>  
        <v-list dense max-width="500">
          <v-header class="text--red text--accent4 font-weight-black my-5">{{$t('bulkLiquidHandling.subtitle2')}}</v-header>
          ...
          >
            <v-list-item
              ...
              target="_blank">
              <v-list-item-content>
                <v-list-item-title v-if="item.name" class="my-1">{{item.name}}</v-list-item-title>
                <v-list-item-title v-else class="my-1">{{item | formatDate}}</v-list-item-title>
                <v-list-item-subtitle>({{item.type}} / {{item.size}})</v-list-item-subtitle>
                <v-list-item-subtitle v-if="item.updated">{{$t('updated')}}: {{item.fecha }}</v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-icon>
...
export default {
  ...
  methods: {
     ...mapActions([ "GetAnpDocuments","UpdateYears", "GetHistoricalMovements","UpdateMonths", "ResetState"]),
    async getDocuments (){
