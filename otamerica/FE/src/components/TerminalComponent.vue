      ...
      <v-row no-gutters>
        <v-col cols="12" lg="9">
                  ...
                  <v-row>
                    <v-col>
                      ...
                      <div v-if="TankTypes.length > 0">
                        ...
                        <p>
                          <span v-for="(value,key) in TankTypes" :key="key">
                            {{$t('tankTypes.'+value.lang_key)}}<span v-if="key+1 != TankTypes.length">, </span>
                          </span>
                        </p>
                      </div>
                      <div v-if="Data.tank_sizes_min">
                        ...
                        <p>{{ Data.tank_sizes_min.toLocaleString() }} <span v-if="Data.tank_sizes_max"> - {{ Data.tank_sizes_max.toLocaleString() }}</span> cbm</p>
                      </div>
                    </v-col>
                    <v-col>
                      <div v-if="AccessTypes.length > 0">
                        ...
                        <p>
                          <span v-for="(value,key) in AccessTypes" :key="key">
                            {{$t('accessTypes.'+value.lang_key)}}<span v-if="key+1 != AccessTypes.length">, </span>
                          </span>
                        </p>
                      </div>
                      <div v-if="Products.length > 0"> 
                        ...
                        <p>
                          <span v-for="(value,key) in Products" :key="key">
                            {{$t('products.'+value.lang_key)}}<span v-if="key+1 != Products.length">, </span>
                          </span>
                        </p>
                      </div>
                      <div v-if="Services"> 
                  ...
                  <v-row>
                    <v-col>
                      <div v-if="Data.tank_capacity">
                        ...
                        <p>{{ Math.round((Data.tank_capacity * 6.29)).toLocaleString() }} bbl</p>
                      </div>
                      <div v-if="Data.tanks >= 0">
                      ...
                      <div v-if="Data.tank_sizes_min">
                        ...
                        <p>{{ Math.round((Data.tank_sizes_min * 6.29)).toLocaleString() }}<span v-if="Data.tank_sizes_max"> - {{ Math.round((Data.tank_sizes_max * 6.29)).toLocaleString() }}</span> bbl</p>
                      </div>
                    </v-col>
                    <v-col>
                      <div v-if="AccessTypes.length > 0">
          ...
          <v-row v-if="Data.marimeFacilities.length > 0" class="my-4">
              ...
              <ul>
                <li v-for="item in Data.marimeFacilities" :key="item.id">
                  ...
                  <span v-if="item.dwt">- <span class="blue--text  text--accent-4">DWT:</span> {{item.dwt.toLocaleString()}} tons</span> <span v-if="item.displacement"> ({{ $t('maritimeFacility.displacement') }})</span>
                  <span v-if="item.draught"> -  <span class="blue--text  text--accent-4">{{ $t('terminalPage.draught') }}:</span> {{item.draught.toLocaleString()}} m</span>
                  <span v-if="item.loa"> - <span class="blue--text  text--accent-4"> {{ $t('terminalPage.loa') }}:</span> {{item.loa.toLocaleString()}} m</span> </li>
              </ul>
          </v-row>
          <v-row v-if="Data.id === 3" class="my-4">
              <a href="#/regulations">{{ $t('regulations.title') }}</a>
          </v-row>
          <div v-if="Data.terminal_data">
            <v-icon class="mr-1">mdi-file-pdf-box</v-icon>
            <a :href="`${appPath}fileDownload.php?name=${Data.terminal_data}`" target="_blank">{{ $t('downloadFactSheetPdf') }}</a>
          </div>
          <ul class="my-4">
            ...
            <li v-if="Data.id === 17">
              <a :href="`${appPath}fileDownload.php?name=peru/doc-0020-lqs-politica-del-sig.pdf`"  target="_blank">{{ $t('integratedManagementSystemPolicy') }}</a>
            </li>
            <li v-if="Data.id === 19">
              <a :href="`${appPath}fileDownload.php?name=peru/1.-politica-del-sistema-integrado-de-gestion.pdf`"  target="_blank">{{ $t('integratedManagementSystemPolicy') }}</a>
            </li>
          </ul>
          <ul class="my-4">
            <li v-for="item in Data.documents"
                :key="item.name">
              <a :href="`${appPath}fileDownload.php?name=${item.url}`"  target="_blank">{{ $t(item.name) }}</a>
            </li>
          </ul>
          ...
          <div v-if="Data.certifications.length > 0">
            ...
            <ul>
              <li v-for="cert in Data.certifications"
                  :key="cert.name">
                <a :href="`${appPath}fileDownload.php?name=${cert.url}`"  target="_blank">{{ $t(cert.name) }}</a>
              </li>
            </ul>
          </div>
        </v-col>
        <v-col cols="12" md="6" lg="3">
          ...
          <div v-if="(Data.id === 9 || Data.id === 8)">
            ...
            <div class="m-contact">
              ...
              <span>
                <v-icon small class="mr-2">mdi-email-outline</v-icon>
                <a :href="`mailto:brasil@otamerica.com`">brasil@otamerica.com</a>
                <br/>
              </span>
            </div>
            ...
            <div class="m-contact">
              ...
              <span>
                <v-icon small class="mr-2">mdi-email-outline</v-icon>
                <a :href="`mailto:mariana.chodera@otamerica.com`">mariana.chodera@otamerica.com</a>
                <br/>
              </span>
            </div>
          </div>
